...
data(kg.hg19)
kg.gr <- GRanges( seqnames = kg.hg19$chr, ranges = IRanges( start = kg.hg19$start, end = kg.hg19$end ), strand = kg.hg19$strand )
@ 
...
for( locus in names(loci) ){
  ...
  if( nrow(kg.sub.df) != 0 ){
    ...
    grtrack <- GeneRegionTrack(kg.sub.df, genome="hg19", chromosome=as.character(seqnames(loci)[which(names(loci)==locus)]), name="Known Genes")
